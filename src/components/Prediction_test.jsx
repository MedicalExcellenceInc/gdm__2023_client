...
const Prediction = () => {
  ...
  const [ lymphocyt, setLymphocyt] = React.useState("");                          // lymphocyt (20230314 추가)
  const [ neutrophil, setNeutrophil] = React.useState("");                        // neutrophil (20230314 추가)
  ...
  const [ hdl, setHdl] = React.useState("");                                      // HDL (HDL cholesterol)
  const [ ldl, setLdl] = React.useState("");                                      // LDL (LDL Cholesterol) (20230314 추가)
 ...
 */
  ...
  /**
   * lymphocyt : 범위 0~100. 소수점 1자리.
   * @param {*} e 
   */
   const onChangeLymphocyt = (e) => {
    const value  = trim(e.target.value);
    setLymphocyt(value);
  ...
  const onBlurLymphocyt = (e) => {
    const value = trim(e.target.value);

    if(checkNumberRage(value, 0, 100)) {
      setLymphocyt(checkNumberPoint(value, 1));
      setLymphocytMsg("");

    }else {
      setLymphocytMsg("0~100 사이 소수점 한자리까지만 입력해주세요.");
      setLymphocyt("");
    }
  }
  ...
  /**
   * Neutrophil : 범위 0~100. 소수점 1자리.
   * @param {*} e 
   */
   const onChangeNeutrophil = (e) => {
    const value  = trim(e.target.value);
    setNeutrophil(value);
  ...
  const onBlurNeutrophil = (e) => {
    const value = trim(e.target.value);

    if(checkNumberRage(value, 0, 100)) {
      setNeutrophil(checkNumberPoint(value, 1));
      setNeutrophilMsg("");

    }else {
      setNeutrophilMsg("0~100 사이 소수점 한자리까지만 입력해주세요.");
      setNeutrophil("");
    }
  }
   ...
   */
   const onChangeLdl = (e) => {
    ...
    if(value !== ''){
      if(validationNum(value)){
        if(checkNumberRage(value, 0, 500)) setLdl(value);
        else {focus("0~500 사이 정수만 입력해 주세요.", getObject("ldl"));}
      }else{
        setLdl(getOnlyNumber(value));
      }
      
    }else setLdl(getOnlyNumber(value));
  }
  ...
  const checkBasicData = (date, hospital, idCode, motherAge, crntGestWeeksW, crntGestWeeksD, edc, gestCnt, ftpn, pbmh, naturalMcCnt, artificialMcCnt   ) => {
    ...
    // 항목 입력 여부 확인
      if(date === ""
        ...
        || crntGestWeeksD === ""
        || edc === ""
        //|| gestCnt === ""          // 20230313 삭제됨
        || ftpn === ""
        || pbmh === ""
        //|| naturalMcCnt === ""     // 20230313 삭제됨
        //|| artificialMcCnt === ""   // 20230313 삭제됨
 ...
 */
  ...
  const checkData = async(value) => {
    ...
    if(value === 'E0'){
      ...
      // M1 필수 항목 (20230313 내용 변경 됨.)
      // 고지혈증, 기타 내분비질환, 자궁근종 개수, 당뇨병, WBC, Lymphocyt, Neutrophil, HCT, Total Cholesterol, HDL Cholesterol, LDL Cholesterol
      // AST, ALT, FBS, 50g GTT, Randome glucose, HbA1C , hCG
      ...
      // (20230313 내용 변경 됨.) 필수항목에서 빠지게 되어 주석처리함.
  ...
  const getResult = async() => {
    ...
    const data = {
      ...
      ftpn: Number(ftpn),
      wbc: Number(wbc),
      lymphocyt: Number(lymphocyt),
      neutrophil: Number(neutrophil),
      hb: Number(hb),
      hct: Number(hct),
      plt: Number(plt),
      tc: Number(tc),
  ...
  const saveData = async(apiResult, apiAiVersion, immuneDurMonths) => {
    console.log('======================== saveData');
      const data = {
          ...
          map: map,
          smoking: smoking,
          vpg_dur: vpgDur === '' || vpgDur=== null ? 0:vpgDur ,
          survch: survch,
          csec: csec,
          ...
          myomano: myomano === '' || myomano === null ? 0 : myomano,
          pcos: pcos,
...
<hr className="prediction-main_sub_border_style" />
    ...
    <div className="prediction-main-item">
      ...
      <div className="prediction-main-item">
          <div className="left-bold">과거임신 총 횟수<span className="left-red-point" >●</span></div>
          <div className="right">
            <input
              id="gestCnt"
              value={gestCnt}
...
<div id='multiparousDiv' >
        ...
        <div className="prediction-main-item">
          <div className="left-bold">생존아수<span className="left-red-point" >●</span></div>
          <div className="right">
            <input
              id="survch"
              value={survch}
        ...
        <div className="prediction-main-item">
          ...
          <div className="right">
            <input
                  id="lymphocyt"
                  value={lymphocyt}
                  onChange={onChangeLymphocyt}
                  onBlur={onBlurLymphocyt}
                  maxLength="5"
                  placeholder="0~100"
                  type="number"
                />
        ...
        <div className="prediction-main-item">
          ...
          <div className="right">
            <input
                  id="neutrophil"
                  value={neutrophil}
                  onChange={onChangeNeutrophil}
                  onBlur={onBlurNeutrophil}
                  maxLength="5"
                  placeholder="0~100"
                  type="number"
                />
